...
{
    TOID(struct hash_node) node;
    int not_used;
    POBJ_NEW(pop, &node, struct hash_node, hash_node_constr, &not_used);
...
static int
hash_constr(PMEMobjpool *pop, void *ptr, void *arg)
{
    if (arg)
        ;
    ...
    for (size_t i = 0; i < size; i++) {
        for (int j = 0; j < BUCKET_SLOTS; j++) {
            walk->keys[j] = NULL_STR;
            walk->values[j] = NULL_STR;
        }
...
new_hash(PMEMobjpool *pop, size_t size)
{
    ...
    size_t total = size * sizeof(struct hash_node);
    POBJ_ALLOC(pop,
               &hash_ptr->table,
               struct hash_node,
               total,
...
const char *
hash_get_value(TOID(struct hash) h, const char *key)
{
    struct hash *h_ptr = D_RW(h);
    size_t key_size = strlen(key);
    unsigned hash_key = jenkins_hash(key, key_size, *(unsigned*)key);
...
hash_remove_item(PMEMobjpool *pop, TOID(struct hash) h, const char *key)
{
    ...
    if (pre->full != EMPTY) {
        for (int i = 0; i < BUCKET_SLOTS; i++) {
            if (!TOID_IS_NULL(pre->keys[i]) && strcmp(key,
                                                      str_get(&pre->keys[i]))==0) {
                str_free(&pre->keys[i]);
                pre->keys[i] = NULL_STR;
    ...
    for(; cur; pre = cur, cur = D_RW(pre->next)) {
        for (int i = 0; i < BUCKET_SLOTS; i++) {
            if (TOID_IS_NULL(cur->keys[i]))
                continue;
            if(strcmp(str_get(&cur->keys[i]), key) == 0) {
                str_free(&cur->keys[i]);
                str_free(&cur->values[i]);
                cur->full--;
            }
            if (cur->full == EMPTY) {
                pre->next = cur->next;
    ...
    for (size_t i = 0; i < h_ptr->size; i++) {
        ...
        while(p) {
            for (int j = 0; j < BUCKET_SLOTS; j++) {
                if (!TOID_IS_NULL(p->keys[i]))
                    printf("%s=>%s, ",
                           str_get(&p->keys[i]), str_get(&p->values[i]));
            }
